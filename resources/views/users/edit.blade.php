        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-primary text-white py-3">
                <h5 class="card-title mb-0 fw-bold">
                    <i class="bi bi-person-gear me-2"></i>{{ __('Edit User') }} — {{ $user->name }}
                </h5>
            </div>
            ...
            <div class="card-body p-4">
                <form action="{{ route('users.update', $user->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        <label for="password" class="form-label fw-semibold">{{ __('New Password') }} <span class="text-muted fw-normal">{{ __('(leave blank to keep current)') }}</span></label>
                        <div class="input-group">
